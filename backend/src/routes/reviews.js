...
router.post('/:reserve_id', protectRoute, requireVerifiedEmail, (req, res) => {
    ...
    try {
        ...
        if (existing) {
            return res
                .status(409)
                .json({success: false, error: 'Вы уже оставили отзыв для этого объекта.'});
        }
...
router.delete('/:id', protectRoute, requireVerifiedEmail, (req, res) => {
    ...
    try {
        ...
        if (review.user_id !== req.user.userId) {
            return res.status(403).json({success: false, error: 'Вы можете удалить только свой отзыв.'});
        }
        ...
        return res.json({success: true, data: {message: 'Отзыв удален.'}});
    } catch (error) {
        console.error(error);
        return res.status(500).json({success: false, error: 'Internal server error'});
    }
